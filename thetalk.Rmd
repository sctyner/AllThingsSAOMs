...
Likelihood function first presented in @snijders2010, based on the augmenting data principle of @dataug. 

SAOMs are "too complicated for the calculation of likelihoods or estimators in closed form" [@snijders2010, p. 568]

Write likelihood in augmented data form instead

## Augmenting Data

For timepoint $m$, consider the transition from $x(t_{m-1})$ to $x(t_m)$. 

- $C_m$: the number of tie differences, $C_m = \sum_{ij}|x_{ij}(t_m) - x_{ij}(t_{m-1})|$
- CTMC: $x(t_{m-1}) = X(T_0), X(T_1), \dots, X(T_{C_m}) = x(t_{m})$
- 
