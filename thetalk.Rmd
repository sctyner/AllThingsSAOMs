...
Structural properties of graph become sufficient statistics, $s_k$
...
set.seed(12345)
ggplot() + 
  geom_net(data = net, label = T, size = 6, 
...
- $\psi(\boldsymbol{\beta})$: the normalizing constant. 
...
where $x$ is the network state, $s_{ik}$ are network statistics, $\boldsymbol{\beta}$ are parameters, and $\mathbf{Z}$ are node-level covariates
...
ggplot(data = net) + 
  ...
  theme_net() + theme(legend.position = 'bottom', panel.background = element_rect(fill = NA, color = 'black'))
```
...
- Accepted for publication in *The R Journal* subject to revision
...
- What makes two network models different?
    ...
    + What is an "average" network from a model given a set of $s_{ik}$s? 

## Research Plan

- Visualization of fitting process of SAOMs
    + Visualization of underlying CTMC 
...
- Use visual tools to diagnose:
    + effect of parameters in SAOM models
    + goodness-of-fit of SAOM and other network models
...
Can we *see* the effect of an added significant parameter when visualizing networks simulated from two different models?
...
Given data and a plot of it, what can you see? 

$H_0$: a null hypothesis scenario from which the data could have arisen 

- Sample from the null hypothesis, make the same plot type as the data plot (create the "null plots")
...
- $M-1$ null plots vs. $1$ data plot (alternative)
...
True [second wave](#41) of [data](#44) among 5 samples simulated from M1 for timepoint 2.
...
## Goodness of Fit for M2

True [second wave](#41) of [data](#44) among 5 samples simulated from M2 for timepoint 2.
...
<img src="VisInfDocs/Results/Trial1.png" height="350px" />

## Trial Plot 2

Most complex 

<img src="VisInfDocs/Results/Trial2.png" height="350px" />
...
p1 <- ggplot(data = res4.2) +geom_point(aes(x = jtt, y = perc_corr)) + labs(x = "Number of JTTs in Alternate Plot", y = "Percent of Participants Choosing Alternate Plot", title= "# of JTTs")
res3a <- res3
...
p2 <- ggplot(data = res5.2) + geom_point(aes(x = param_est, y = perc_corr)) + labs(x = "Parameter estimate", y = "Percent of Participants Choosing Alternate Plot", title = "Parameter Estimate")
gridExtra::grid.arrange(p2, p1, nrow = 1)
```

## Results 2 {.smaller}
Gray fill incates identification of data plot

``` {r, echo=FALSE, fig.align = 'center', out.width = '90%',fig.height=3}
# get the weights
...
plotss <- unique(jttres$group_name)
jttplots <- NULL
for(i in 1:length(plotss)){
  dat <- jttres2 %>% dplyr::filter(group_name == plotss[i])
  jttplots[[i]] <- ggplot(data = dat, 
...
#  facet_wrap(~group_name, scales = "free", ncol = 5) +
  ...
  labs(x = "User Response" , y = "Number of Responses", title = plotss[i])
}
gridExtra::grid.arrange(jttplots[[1]], jttplots[[2]], jttplots[[3]], jttplots[[4]], jttplots[[5]], nrow = 1)
```
...
## Results 3 {.smaller}
Gray fill incates identification of data plot
```{r, out.width="90%", fig.height=5, fig.align='center'}
m1m2res <- as.data.frame(filter(summ_res_wts, group_name %in% groupss[c(6:10, 16:20)]))
m1m2res <- m1m2res[-36,]
m1m2res[36,4] <- 6+.5
plotss2 <- unique(m1m2res$group_name)
m1m2plots <- NULL
for(i in 1:length(plotss2)){
  dat <- m1m2res %>% dplyr::filter(group_name == plotss2[i])
  m1m2plots[[i]] <- ggplot(data = dat, 
       aes(x = reorder(as.factor(response_no2), -tot_wt), 
                                y = tot_wt,
                                fill = correct)) + 
  geom_bar(color = 'black', stat = 'identity') + 
...
#  facet_wrap(~group_name, scales = "free", ncol = 5) +
  ...
  theme(legend.position = 'none', axis.text = element_text(size = 8),
        axis.title = element_text(size = 10)) + 
  labs(x = "User Response" , y = "Number of Responses", title = plotss2[i])
}
gridExtra::grid.arrange(m1m2plots[[1]], m1m2plots[[2]], m1m2plots[[3]], m1m2plots[[4]], m1m2plots[[5]], m1m2plots[[6]], m1m2plots[[7]], m1m2plots[[8]], m1m2plots[[9]], m1m2plots[[10]], nrow = 2, ncol = 5)
```

## What have we learned so far?

Visual goodness of fit tests feasible

High value of jumping transitive triplet statistic is visible. 
