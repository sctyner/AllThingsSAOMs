...
Many possible structures, $s_{ik}(x, \mathbf{Z})$, to add, belong to two groups: 
...
Write likelihood in augmented data form instead, using a constant rate function (more detail in [Appendix A](#66))
...
names(ddf) <- c("id", "wave","drink")
ddf$wave <- as.factor(ddf$wave)
levels(ddf$wave) <- 1:3
ddf$wave <- as.numeric(as.character(ddf$wave))
fdf <- filter(fdf, value == 1)
net <- full_join(fdf, ddf, c("from"="id" ,"wave"="wave"))
ggplot(data = net) + 
  geom_net(aes(from_id = from, to_id = to, color = as.factor(drink)), directed = T, fiteach = T, size = 2, arrowsize = .3) + 
  scale_colour_brewer(palette = "Spectral",name = "Covariate") + 
  facet_wrap(~wave) + 
...
- Point shapes, colors, sizes can indicate node variables
- Segment size, color, linetype can indicate edge variables
...
- What does the underlying continuous time Markov chain look like? - What makes two network models different?
...
- Different rate for each time point, $\alpha_1, \alpha_2$
...
3. Jumping Transitive Triplets ($\beta_3$) - friends of the same person become friends despite behavior differences
...
- $p$-value: $Pr(X \geq x) = 1 - Binom_{K, \frac{1}{M}}(x-1)$
    + $X$: number of observers who correctly identify the data
...
![](VisInfDocs/Results/Directions.png)
...
- Had to correctly answer two trials to qualify for the experiment
