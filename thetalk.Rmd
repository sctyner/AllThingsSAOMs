...
## Basic Definitions & Notation {.smaller}
...
Created to model networks more realistically

For directed or undirected graphs. Also called $p^*$ models [@p1model]. 
...
- Likelihood in augmented data form using a constant rate function presented in detail in [Appendix A](#66))
...
jttres <- data.frame(filter(summ_res_wts, group_name %in% groupss[1:5]) %>% ungroup())
ggplot(data = jttres, 
       aes(x = reorder(response_no2, -tot_wt), 
                                y = tot_wt,
                                fill = correct)) + 
