...
- Probability that one observer picks the alternative plot randomly is $\frac{1}{m}$
...
ggplot(data = filter(summ_res_wts, group_name %in% groupss[1:10]), 
...
ggplot(data = filter(summ_res_wts, group_name %in% groupss[11:25]), 
       aes(x = reorder(as.factor(response_no2), -tot_wt), 
                                y = tot_wt,
                                fill = correct)) + 
  geom_bar(color = 'black', stat = 'identity') + 
