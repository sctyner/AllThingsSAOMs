...
## Basic Notation {.smaller}

- Observed network: $x$
- Unobserved network (random variable): $X$
- Space of possible networks given $n$ nodes: $\mathcal{X}$ 
- Number of possible directed networks given $n$ nodes: 
$$|\mathcal{X}|=2^{n(n-1)}$$
- Nodes: $\mathcal{N} = \{1,\dots, n\}$, where $n=$ number of nodes in network, $\mathcal{N}$ is nodeset
- Edges: Tie between nodes $i$ and $j$ is $x_{ij}$, for $i \neq j \in \mathcal{N}$. Edgeset is $\mathcal{E} = \{x_{ij}\}_{i \neq j \in \mathcal{N}}$
    + Tie between $i$ and $j$ exists $\rightarrow$ $x_{ij} = 1$. Is $0$ otherwise
    + Example has binary, directed edges: $x_{12} = 1 \neq x_{21} = 0$ 

## Erdös-Rényi Simple Random Graph Model
For *undirected* graphs. First random graph model [@er59]. 

- Start with $n$ nodes
- Ties $x_{ij}$ chosen at random from the $\binom{n}{2}$ ties possible
- $Pr(x_{ij} = 1) = p$ for any $i \neq j \in \{1, \dots, n\}$
- Very simple, can easily write down a likelihood and estimate parameters
...
For directed or undirected graphs. Also called $p^*$ models [@p1model]. Created to model networks more realistically
...
```{r exs, echo=FALSE, out.width="80%", fig.height=3, fig.align='center'}
...
set.seed(12345)
ggplot() + 
  geom_net(data = net, label = T, size = 8, 
...
Recall: $\boldsymbol{\beta}$ correspond to the $s_{ik}(x)$. 

There should be two $\beta_k$ at minimum [@snijders01 p. 371]:
- $\beta_1$: network popularity (outdegree)
- $\beta_2$: reciprocity

Many possible parameters $\beta$ to add, belong to two groups: 
...
Likelihood function first presented in @snijders2010, based on the augmenting data principle of @dataaug. 

SAOMs are "too complicated for the calculation of likelihoods or estimators in closed form" [@snijders2010, p. 568]

Write likelihood in augmented data form instead, using a constant rate function
...
2. The joint probability of the sample path occurring under the model's rate of change and objective function
    $$\prod_{c = 1}^{C_m} \alpha_m p_{i_cj_c}(\boldsymbol{\beta}, X(T_{c-1}))$$ 
...
2. Using definitions from previous slides:
    $$\alpha_m^{C_m} \prod_{c=1}^{C_m} \frac{\exp\left\{f_{i_c j_c}(\boldsymbol{\beta}, X(T_{c-1}))\right\}}{\sum_{j \neq j_c}\exp\left\{f_{i_cj}(\boldsymbol{\beta}, X(T_{c-1}))\right\}}$$
where $f_{ij}(\boldsymbol{\beta}, x)$ denotes the value of the objective function when the tie $x_{ij}$ changes 
...
## Friendship Network {.smaller}

Small, $n = 16$ subset of data from @friendsdata. 

- Three network waves describing friendships between teenage girls in a Scottish school
...
## Fitting SAOMs to the Data

Observed friendship networks: discrete observations of continuous time Markov Chain. 

1. Between observations:  actors act (i.e. change ties with others) at same rate, $\alpha_m$
2. Each girl aims to maximize her objective function, $f_i(\mathbf{\beta},x) = \sum_k \beta_k s_{ik}(x)$.

## Model Parameters

1. Rate ($\alpha_1, \alpha_2$) - how often girls change friendships in between observations 
2. Outdegree ($\beta_1$)- measure of activeness
3. Reciprocity ($\beta_2$) - how often girls' friendships are reciprocated
4. Jumping Transitive Triplets ($\beta_3$) - friends of the same person become friends despite behavior differences
...
When looking at networks simulated from two different models, is there a visible difference due to the model differences?
...
- Hypothesis: If tests of individual parameters $\beta_k$ tell find a significant difference in two models, then networks simulated from those models should look different to us.
- Method: Visual Inference - Using the lineup protocol of @Bujaetal, show networks simulated from 2 models, differing by one parameter $\beta_k$ to test this hypothesis

## Visual Inference

Given data and a plot of it, what can you say about the data? 

$T^{(i)}(\mathbf{y})(i \in I)$: the set of discoverable features of a plot are test statistics, where $\mathbf{y}$ is the data, and $I$ is the set of all possible visual features to discover in a plot. 
...
- $p$-value: $Pr(X \geq x) = 1 - Binom_{K, \frac{1}{M}}(x-1)$
    + $X$: number of analysts who correctly identify the data
    + $x$: the observed value $X$ for an experiment
    + $Binom_{K, \frac{1}{M}}(x)$: pmf of the binomial distribution with $K$ trials and probability of success $\frac{1}{M}$ evaluated at $x$.
...
<img src="VisInfDocs/Results/Ex1.png" width="700px" height="450px" />
...
ggplot(data = filter(summ_res_wts, group_name %in% groupss[1:5]), 
...
ggplot(data = filter(summ_res_wts, group_name %in% groupss[c(6:10, 16:20)]), 
       aes(x = reorder(as.factor(response_no2), -tot_wt), 
                                y = tot_wt,
                                fill = correct)) + 
  geom_bar(color = 'black', stat = 'identity') + 
...
Jumping transitive triplet statistics is visible. 
...
- Use visual tools to diagnose the effect of additional parameters in SAOM models
- Learn about the *practical* relevance of effects
- Perform model selection using visual inference methods

Why?

- Many effects to choose from (88 at last count in `RSiena` software)
- Significance tests exist, but have practical limitations. 
    + Do they have a visible effect on structure of the network?
    + Example: jumping transitive triplets. In high numbers, have large effect on appearance, high parameter value not as effective as the statistic
...
- Target audience: "tidyverse" package users
...
- What does the underlying process look like? 
- What is an "average" network from a model given parameters? 
...
- 1000 simulations from M1 as estimated in Chapter 2. 
- Edge appearances: mean: 59.21; median: 21
