...
## Examples of Statistics {.smaller}
...
Objective function also has a random component, $U_i(x)$ 

- Gumbel / log-Weibull distribution with mean $\mu = 0$ and scale parameter $\sigma = 1$ [@snijders01, p. 368].
- probability distribution function:
$$f(u|\mu, \sigma) = \frac{1}{\sigma}\exp\left\{-\left(\frac{u-\mu}{\sigma} + e^{-\frac{u-\mu}{\sigma}}\right)\right\} $$

This is used so that the *probablity* $i$ changes tie $x_{ij}$ can be written in terms of $f_i(\boldsymbol{\beta}, x)$: 
...
## CTMC Model Description {.smaller}

CTMCs described by their intensity matrix, $\mathbf{Q}$, which dictates rates of change between chain states

Large number of network states: $2^{n(n-1)}$, but only $n(n-1)$ accessible by changing 1 tie at a time.

$\mathbf{Q}$ very sparse. Let $b \neq c \in \{1, 2, \dots, 2^{n(n-1)} \}$ be indices of two different possible states of the network, $x^b, x^c \in \mathcal{X}$.  Then the $bc^{th}$ entry of $\mathbf{Q}$ is:
\[ q(x^b, x^c) = \begin{cases} 
      q_{ij}(\alpha, \rho, \boldsymbol{\beta}, x^b) & \text{if } x^c \in \{x^b(i \leadsto j) | \text{ any } i \neq j \in \mathcal{N}\} \\
...
where 
    $$q_{ij}(\alpha, \rho, \boldsymbol{\beta}, x^b) = \lambda_i(\alpha, \rho, x^b, m)p_{ij}(\boldsymbol{\beta}, x^b)$$ 

## Model Fitting

Done in the R package `RSiena` by @RSiena.

MCMC methods used for method of moments estimators of the parameters.
