...
```{r directions, out.width="50%", fig.cap="The welcome message shown to the Amazon Turk users."}
...
We collected on ten lineups each from 77 Amazon Turk users. A table of demographic information collected on the participants in the experiment is in Table \@ref(fig:demos). 
```{r demos}
...
ggplot(data = res2, aes(x = conf_level, y = log(time))) + geom_boxplot() + coord_flip() + theme_bw()
ggplot(data = res2, aes(x = correct, y = log(time))) + geom_boxplot() + coord_flip() + theme_bw()
ggplot(data = res2, aes(x = log(time))) + 
  geom_histogram(binwidth = .5, fill ='white', color = 'black') +
  facet_wrap(~correct, nrow = 2, scales = 'free_y') + theme_bw()
