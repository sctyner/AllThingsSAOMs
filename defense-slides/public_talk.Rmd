...
```{r densrecipex, dev.args=list(bg='transparent'), fig.height=5, fig.show='hold', out.width = '49%'}
...
```{r bigmodex, fig.align='center', fig.height=5.5, fig.width=9}
...
- Alternative model is 5
- Most people picked 4 or 5
...
- M2 for 3 values of $\beta_3 \leq \hat{\beta}_3$ and 3 values of $\beta_3 > \hat{\beta}_3$
- M3 for 3 values of $\beta_4 \geq \hat{\beta}_4$
- M4 for 3 values of $\beta_5 < \hat{\beta}_5$ and 3 values of $\beta_5 > \hat{\beta}_5$
- M5 for 3 values of $\beta_6 > \hat{\beta}_6$

---

# Hypothesized power for an additional parameter
...
# Hypothesized power for changing M1 parameters
...
For M3 and M5, smaller values of $\beta_4$ and $\beta_6$ were unnoticeable.
...
**Alternative model** for generating 1 alternative plot in the lineup: M1 ( $\beta_1 = \hat{\beta}_1, \beta_2 =\hat{\beta}_2$ )
...
experdet$Condition[c(5,11)] <- "reverse" 
...
All but 2 values $\alpha_{ij}$ were significant at 0.05

All but 3 values $\beta_{ij}$ were significant at 0.05
...
densneg1 <- seq(max(filter(newdata, type2 == -1, test_param == "dens")$size), -4.902777, by = filter(newdata, type2 == -1, test_param == "dens")$size[2:3] %>% diff)
denspos1 <- seq( -4.902777, min(filter(newdata, type2 == 1, test_param == "dens")$size), by = filter(newdata, type2 == 1, test_param == "dens")$size[2:3] %>% diff)
newdata2 <- data.frame(test_param = "dens", 
...
ggplot() + 
  geom_line(data = newdata %>% filter(test_param == "dens"), aes(x = size, y = predictfinal, color = as.factor(type2))) + 
  ...
  theme_bw() + 
  theme(legend.position = 'bottom', strip.text = element_text(size = 20), text = element_text(size = rel(7)), legend.text = element_text(size = rel(7)), legend.key.size = unit(1, "inches"))
```
