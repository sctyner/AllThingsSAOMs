...
knitr::opts_chunk$set(echo = FALSE, tidy = TRUE, message = FALSE, warning = FALSE, dev = 'png', cache = TRUE)
```
...
- Start with a "data plot": a visualization of the data of some aspect of interest
- Null hypothesis with a corresponding "null model"

$H_0$: data is consistent with null model
$H_A$: data is not consistent with null model

- Lineup: 
    + **Null plots**: plots of data generated according to the null model
    + **Data plot**: randomly insert the data plot among a set of null plots
...
- Simulate network data from models of interest and place them in various lineup configurations to determine:
...
- Use those estimates to simulate from the models
...
Binary response variable $Y_{ijkm}$ indicating whether participant $m$ selected the alternative data plot in the $k^{th}$ repetition of a lineup of parameter $i$ and type $j$. 

$$Y_{ijkm} \sim \text{Bernoulli}(\pi_{ijkm})$$
$$\text{logit}(\pi_{ijkm}) = \alpha_{ij} + \gamma_{ij} x + \delta_{ijk} + \epsilon_{m}$$
...
# Predicting for new repetition, new observer
...
$$E[\pi_{ij}] = \frac{\exp\{\alpha_{ij} + \gamma_{ij} x\}}{1 + \exp\{\alpha_{ij} + \gamma_{ij} x\}}$$
...
- Only discussing results for visual power 
- Significance test, goodness-of-fit test results not shown in this talk
...
8 of 12 $\gamma_{ij}$ were significant at 0.05
...
- $*-(\alpha_{51}, \beta_{51})$ have $p$-values (0.051, 0.063)
...
ggplot() + 
  ...
  geom_point(data = plotdata %>% filter(test_param=="simttb"), aes(x = size, y = datapick, color = as.factor(type2)), size = 4) + 
  ...
  geom_line(data = newdata %>% filter(test_param == "simttb", category == "outside2"), aes(x = size, y = predictfinal, group = as.factor(type2)), color = "gray60", size = 1.5, alpha = .7) +
  ...
  scale_color_manual(values = c("#4c0826", "#01456d"),name = "Lineup Condition", labels = c("-", "+")) +
  facet_grid(~param) + 
...
???
Remember triangles! Positive = lots of triangles, negative = no triangles, sparse graph
...
ggplot() + 
  ...
  geom_point(data = plotdata %>% filter(test_param=="dens"), aes(x = size, y = datapick, color = as.factor(type2)), size = 4) + 
  ...
  geom_line(data = newdata %>% filter(test_param == "dens", category == "outside2"), aes(x = size, y = predictfinal, group = as.factor(type2)), color = "gray60", size = 1.5, alpha = .7) +
  #geom_text(data = plotdata, aes(x = size, y = datapick, color = as.factor(type2), label = pic_id)) +
  #geom_text(data = labdat, aes(x = x, y=y, label = label, color = as.factor(type2)),
  #          show.legend = F) + 
...
???
Remember hairball! Higher density parameter more connections, lower density parameter fewer connections
...
ggplot() + 
  ...
  geom_point(data = plotdata %>% filter(test_param=="jtts"), aes(x = size, y = datapick, color = as.factor(type2)), size = 4) + 
  ...
  geom_line(data = newdata %>% filter(test_param == "jtts", category == "outside2"), aes(x = size, y = predictfinal, group = as.factor(type2)), color = "gray60", size = 1.5, alpha = .7) +
  #geom_text(data = plotdata, aes(x = size, y = datapick, color = as.factor(type2), label = pic_id)) +
  #geom_text(data = labdat, aes(x = x, y=y, label = label, color = as.factor(type2)),
  #          show.legend = F) + 
  geom_vline(xintercept = 3.340302) + 
  geom_hline(yintercept = 1/6, linetype='dotted') +
  scale_color_manual(values = c("#4c0826", "#01456d"),name = "Lineup Condition", labels = c("reverse", "regular")) +
  facet_grid(~param) + 
  labs(x = "Parameter value", y = "Expected probability of detection in a lineup of size 6") + 
  theme_bw() + 
  theme(legend.position = 'bottom', strip.text = element_text(size = 20), text = element_text(size = rel(5)), legend.text = element_text(size = rel(7)), legend.key.size = unit(1, "inches"))
...
???
Another triangle effect: reverse = all plots have many triangles, want to detect the one without. regular = only one plot has trianges. 
Harder to process when many triangles. 
...
ggplot() + 
  ...
  geom_point(data = plotdata %>% filter(test_param=="recip"), aes(x = sqrt(size), y = datapick, color = as.factor(type2)), size = 4) + 
  ...
  geom_line(data = newdata %>% filter(test_param == "recip", category == "outside2"), aes(x = sqrt(size), y = predictfinal, group = as.factor(type2)), color = "gray60", size = 1.5, alpha = .7) +
  #geom_text(data = plotdata, aes(x = size, y = datapick, color = as.factor(type2), label = pic_id)) +
  #geom_text(data = labdat, aes(x = x, y=y, label = label, color = as.factor(type2)),
  #          show.legend = F) + 
...
ggplot() + 
  ...
  geom_point(data = plotdata %>% filter(test_param=="jttp"), aes(x = size, y = datapick, color = as.factor(type2)), size = 4) + 
  ...
  geom_line(data = newdata %>% filter(test_param == "jttp", category == "outside2"), aes(x = size, y = predictfinal, group = as.factor(type2)), color = "gray60", size = 1.5, alpha = .7) +
  #geom_text(data = plotdata, aes(x = size, y = datapick, color = as.factor(type2), label = pic_id)) +
  #geom_text(data = labdat, aes(x = x, y=y, label = label, color = as.factor(type2)),
  #          show.legend = F) + 
...
ggplot() + 
  ...
  geom_point(data = plotdata %>% filter(test_param=="samettp"), aes(x = size, y = datapick, color = as.factor(type2)), size = 4) + 
  ...
  geom_line(data = newdata %>% filter(test_param == "samettp", category == "outside2"), aes(x = size, y = predictfinal, group = as.factor(type2)), color = "gray60", size = 1.5, alpha = .7) +
  #geom_text(data = plotdata, aes(x = size, y = datapick, color = as.factor(type2), label = pic_id)) +
  #geom_text(data = labdat, aes(x = x, y=y, label = label, color = as.factor(type2)),
  #          show.legend = F) + 
